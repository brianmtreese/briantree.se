...
First up, we have basic [event binding](https://angular.dev/guide/templates/event-listeners?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"}.
...
Sometimes you may find that you need to bind to events on the [host element](https://angular.dev/guide/components/host-elements?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} of a component or directive.
...
It’s really basic, but in that tutorial, we used the [@HostListener](https://angular.dev/api/core/HostListener?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} decorator to listen for those [click events](https://developer.mozilla.org/en-US/docs/Web/API/Element/click_event){:target="_blank"} because that was the right way to do it at the time.

But now, the [@HostListener](https://angular.dev/api/core/HostListener?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} decorator is no longer recommended for use. 

Instead, we should use the [host](https://angular.dev/api/core/Component?utm_campaign=deveco_gdemembers&utm_source=deveco#host){:target="_blank"} metadata property on the component or directive.
...
So don’t use the [@HostListener](https://angular.dev/api/core/HostListener?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} decorator anymore, this is the new way to do it.

## Custom Events: Outputs (Updated)

Now, something else that we were doing in [this directive](https://stackblitz.com/edit/stackblitz-starters-3srm2utq?file=src%2Fhost-listener%2Fhost-listener.directive.ts) was using the [@Output](https://angular.dev/api/core/Output?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} decorator and an [EventEmitter](https://angular.dev/api/core/EventEmitter?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} to emit this custom event:
...
This is because [@Output](https://angular.dev/api/core/Output?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} and [EventEmitter](https://angular.dev/api/core/EventEmitter?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} were the standard for custom events, but now it’s all about the [output()](https://angular.dev/api/core/output?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} function.

So, we can just switch this to the new [output()](https://angular.dev/api/core/output?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} function and we can remove the [EventEmitter](https://angular.dev/api/core/EventEmitter?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} because it’s no longer needed when using this new function:
...
So, don’t use the old [@Output](https://angular.dev/api/core/Output?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} decorator and [EventEmitter](https://angular.dev/api/core/EventEmitter?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} anymore, use the new [output()](https://angular.dev/api/core/output?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} function instead.

## Global Events: Renderer2 Listeners

Okay, now what about listening to global events?

Well, just like the previous tutorial, when we need to listen to global events in Angular, we can still use the [Renderer2](https://angular.dev/api/core/Renderer2?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} class and its [listen()](https://angular.dev/api/core/Renderer2?utm_campaign=deveco_gdemembers&utm_source=deveco#listen){:target="_blank"} method like we’re doing here:
...
The idea here is to use the [Renderer2](https://angular.dev/api/core/Renderer2?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} [listen()](https://angular.dev/api/core/Renderer2?utm_campaign=deveco_gdemembers&utm_source=deveco#listen){:target="_blank"} method, passing in the element you want to listen on, in this case, the body. 
...
When listening to events like this, though, we have to handle the cleanup manually to prevent performance issues, which is what we’re doing here in the [ngOnDestroy](https://angular.dev/api/core/OnDestroy?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} lifecycle hook:
...
Ok, now we have the new kid on the block… [model()](https://angular.dev/api/core/model?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} inputs.

Think of this as [two-way binding](https://angular.dev/guide/templates/two-way-binding?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} for the modern Angular era.

To add this concept, we’ll add a “value” property using the new [model()](https://angular.dev/api/core/model?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} function, and it’ll be typed as a string:
...
Here we’ll add a [template reference variable](https://angular.dev/guide/templates/variables?utm_campaign=deveco_gdemembers&utm_source=deveco#template-reference-variables) to grab the current value, then we’ll wire up our input event to keep the model in sync:
...
A [model()](https://angular.dev/api/core/model?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} input is like an [input](https://angular.dev/guide/components/inputs?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} and an [output](https://angular.dev/guide/inputs-outputs?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} in one, so it keeps the value in sync between the parent and child components.

So, if there was a value coming from the parent it would flow through like an [input](https://angular.dev/guide/components/inputs?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"}. 
...
The first thing we need to do is add a property, let’s call it "formValue", and it will be a [signal](https://angular.dev/guide/signals?utm_campaign=deveco_gdemembers&utm_source=deveco#writable-signals){:target="_blank"} initialized to some default label:
...
Okay, now we can use the "banana in a box" syntax, that's [two-way binding](https://angular.dev/guide/templates/two-way-binding?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"}, to bind to the value [model()](https://angular.dev/api/core/model?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} input:

```html
<app-example [(value)]="formValue"></app-example>
```

It’s important to note that when using [two-way binding](https://angular.dev/guide/templates/two-way-binding?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} like this, we don’t include the parentheses for the signal.

Now, there’s a [slot](https://angular.dev/guide/components/content-projection?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} in this component between the form and the list of events so here we’re going to output the value of this property so we can see how this works as we interact with the form:
...
- Host events now use the [host property](https://angular.dev/api/core/Component?utm_campaign=deveco_gdemembers&utm_source=deveco#host) instead of the old decorator.

- Custom events use the new [output()](https://angular.dev/api/core/output?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} function instead of the old decorator and [EventEmitter](https://angular.dev/api/core/EventEmitter?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"}.

- Global events can still use [Renderer2](https://angular.dev/api/core/Renderer2?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} but it should really be a last resort.

- And then we have the new [model()](https://angular.dev/api/core/model?utm_campaign=deveco_gdemembers&utm_source=deveco){:target="_blank"} input, a modern two-way binding concept with signals.
...
- [Angular: Adding event listeners](https://angular.dev/guide/templates/event-listeners?utm_campaign=deveco_gdemembers&utm_source=deveco)
- [Angular: Binding to the host element](https://angular.dev/guide/components/host-elements?utm_campaign=deveco_gdemembers&utm_source=deveco#binding-to-the-host-element)
- [Model inputs](https://angular.dev/guide/components/inputs?utm_campaign=deveco_gdemembers&utm_source=deveco#model-inputs)
- [Modern Inputs & Outputs in Angular](https://angular.dev/guide/components/inputs?utm_campaign=deveco_gdemembers&utm_source=deveco)
- [Angular Renderer2 Documentation](https://angular.dev/api/core/Renderer2?utm_campaign=deveco_gdemembers&utm_source=deveco)
